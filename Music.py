import os
print("[+] installing dependencies and it will run the code automaticly")
os.system("pip install pygame")
os.system("pip install pytube")
print("[!] version 0.1 build by pratv3")
from tkinter import *
...
def h():
    try:
        ...
        if d==True:
            ms.showinfo("music player","paste the link of video in command prommpt and hit enter")
            print("direct audio converter from youtube to file")
            f=input("give the link fo video which is on top--)")
            if "http" or "https" in f:
                try:
                    audio=YouTube(f)
                    print("details of video to check accuracy")
                    s=audio.title
                    print(f"title of video--){audio.title}")
                    print(f"thumbnail of video--){audio.thumbnail_url}")
                    aud=audio.streams.get_audio_only()
                    aud.download()
                    print("!!sucess!!")
                except:
